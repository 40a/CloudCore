...
import (
	...
	"github.com/CloudPac/CloudCore/exec/sync"
...
func getCmd(j *types.AnsibleJob, socket string, pid int) (cmd *exec.Cmd, cleanup func(), err error) {
	...
	pargs := []string{"-v", "0", "-r", "/",
		...
		"-b", j.Paths.VarLibJobStatus + ":/var/lib/cloudcore/job_status",
	...
	pPlaybook := []string{
		"ansible-playbook", "-i", "/var/lib/cloudcore/plugins/inventory/cloud_core.py",
	}
	...
	cmd.Env = []string{
		...
		"ANSIBLE_CALLBACK_PLUGINS=/var/lib/cloudcore/plugins/callback",
...
func buildParams(j types.AnsibleJob, params []string) []string {
	...
	extras := map[string]interface{}{
		"cloud_core_job_template_name": j.Template.Name,
		"cloud_core_job_id":            j.Job.ID.Hex(),
		"cloud_core_user_id":           j.Job.CreatedByID.Hex(),
		"cloud_core_job_template_id":   j.Template.ID.Hex(),
		"cloud_core_user_name":         "admin",
		"cloud_core_job_launch_type":   j.Job.LaunchType,
	}
