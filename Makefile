...
RPMSPEC = $(RPMSPECDIR)/cloudcore.spec
RPMRELEASE = $(RELEASE)
ifneq ($(OFFICIAL),yes)
...
sdist: vet
	...
	cp packaging/config/cloud-core.conf build/$(NAME)-$(VERSION)/etc/
	cp packaging/systemd/cloud-core.service build/$(NAME)-$(VERSION)/systemd/
	cp -a docs build/$(NAME)-$(VERSION)/
	go build -v -o build/$(NAME)-$(VERSION)/bin/cloudcored ./cloudcored/...
	go build -v -o build/$(NAME)-$(VERSION)/bin/cloudcore ./cmd/...
...
rpm: rpmcommon
	...
	@rm -f build/rpm-build/$(NAME).spec
	@echo "#############################################"
	@echo "CloudCore RPM is built:"
	@echo "    build/rpm-build/$(RPMNVR).noarch.rpm"
	@echo "#############################################"

mongo:
	mongo coredb --eval 'db.createUser({user:"cloudcore",pwd:"cloudcore",roles:["readWrite","dbAdmin" ] });'
	mongo coredb --eval 'db.users.insert({"_id":new ObjectId(),"username":"admin","first_name":"Gamunu",'\
...
# Build CloudCore docker image and tag with current version
docker:
	cd packaging/docker/cloudcore/ && docker build -t cloudpac/cloudcore:$(VERSION) -t cloudpac/cloudcore:latest .
...
# run CloudCore locally using reflex
run:
	reflex -r '\.go$$' -s -- sh -c 'go run cloudcored/main.go'
